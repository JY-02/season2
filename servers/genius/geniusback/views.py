...
class DraftViewSet(viewsets.ModelViewSet):
    ...
    def choose_diff(self, request: Request):
        ...
        try:
            ...
            return Response(
                {"error": "invalid diff_Count. " "must be an integer between 3 and 5."}, status=400
            )
    ...
    def genre(self, request: Request):
        ...
        try:
            ...
            return Response(
                {"error": "회원을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND
            )

        Draft.objects.filter(user=member).update(genre=genre)

        return Response(
            {"message": "Draft 장르가 성공적으로 업데이트되었습니다."}, status=status.HTTP_200_OK
        )
...
class IntroViewSet(viewsets.ModelViewSet):
    ...
    def create_subjects(self, request: Request):
        ...
        return Response(
            {"message": "Intro created successfully", "intros": created_subjects},
    ...
    def recreate_subjects(self, request: Request):
        ...
        for subject in subjects:
            ...
            created_subjects.append({"id": intro.id, "subject": intro.subject})
        return Response(
            {"message": "subject recreated successfully", "intros": created_subjects},
            status=status.HTTP_201_CREATED,
        )
